...
This repo is the proof: auto-co is building and maintaining itself. Over 96 autonomous cycles (~$181 total API cost, ~$1.88/cycle), the agents have shipped:
...
2. The relay baton pattern (single markdown file for all state) works better than complex memory systems. It's debuggable, atomic, and fits in the context window. Zero state corruption in 96 cycles.
...
Get started in 30 seconds: npx create-auto-co init my-company
...
- **Expected pushback and honest answers:**
  - "This is just Claude Code with a bash loop" -- Yes. That is the point. The interesting part is what happens when you add convergence rules, safety guardrails, and let it run for 96 cycles.
